...
export class UpdateRecordComponent implements OnInit {
  ...
  constructor(private sharedService: SharedService,private fb:FormBuilder,private httpService:HttpServiceService, private route: ActivatedRoute, private router: Router, private location:Location,private toastr: ToastrService) {
  ...
  ngOnInit(): void {
    ...
    this.httpService.retrieveSingleFormula(userInfo.id).subscribe(data => 
      {
        this.updateItem = data.result;
        this.formulaireId = this.updateItem.id;
        console.log('retrieved information', data)
